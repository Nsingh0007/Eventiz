...
const Events = ({ events }) => {
  ...
  const [showModal, setShowModal] = useState({ state: false, event: {} });
  const [deleteModal, setDeleteModal] = useState(false);
  const handleRoute = (slug, id) =>
    router.push({ pathname: `/events/${id}/${slug}` });

  const openModal = (e) => setShowModal({ state: true, event: e });
  const closeModal = () => setShowModal({ state: false, event: {} });
  ...
  return (
    <div className="w-full flex flex-col items-center py-[60px] px-[20px]">
      ...
      <div className="w-full flex flex-wrap items-center justify-center">
        ...
        {showModal.state && (
          <ShareEventModal event={showModal.event} closeModal={closeModal} />
        )}
      </div>
    </div>
  );
